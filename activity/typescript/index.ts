...
const response = (person:Person): string | number[] => {
  return Object.values(person);
}
const responseGen = <T>(person:Person): Array<T> => {
  return Object.values(person);
}
...
const someoneKeysGen = response(someone);
console.log("someone gen:", someoneKeysGen);
const someoneValues = response(someone);
...
const responseVehicle = (vehicle: Car | Motorcycle) => {
